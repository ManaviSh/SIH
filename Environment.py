...
class environment():
    ...
    def geocoding_api(self, address):  # 2 output: status, position
        ...
        geocode_url = geocode_api + urllib.parse.urlencode({'address': address,'key': secret_key})
    ...
    def elevation_api(self, location):  # 2 output: elevation, resolution
        ...
        elevation_url = elevation_api + urllib.parse.urlencode({'locations': location,'key': secret_key})
    ...
    def directions_api(self, origin, destination):  # 5 output: status, steps, bound
        ...
        directions_url = directions_api + urllib.parse.urlencode({'origin': origin}) + '&' + urllib.parse.urlencode(
            {'destination': destination}) + '&' + urllib.parse.urlencode({'units': 'metric','key': secret_key})
    ...
    def step(self, action):  # output:
        ...
        return self.current_position, self.step_reward, current_status, self.charge_num, batterySOC, self.energy_consume
